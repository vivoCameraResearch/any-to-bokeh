...
def decode_latents(vae, latents, num_frames, decode_chunk_size=25):
...
def tensor_to_vae_latent(t, vae, cache_flag=False):
    video_length = t.shape[1]

    t = rearrange(t, "b f c h w -> (b f) c h w")
    latents = vae.encode(t, cache_flag=cache_flag).latent_dist.sample()
...
def sobel_xy(x):
    ...
    sobel_x = torch.tensor([[1, 0, -1], [2, 0, -2], [1, 0, -1]],
                           dtype=torch.float32, device=x.device).view(1, 1, 3, 3).repeat(C, 1, 1, 1)
    sobel_y = torch.tensor([[1, 2, 1], [0, 0, 0], [-1, -2, -1]],
                           dtype=torch.float32, device=x.device).view(1, 1, 3, 3).repeat(C, 1, 1, 1)
    gx = F.conv2d(x, sobel_x, padding=1, groups=C)
    gy = F.conv2d(x, sobel_y, padding=1, groups=C)
    return torch.sqrt(gx**2 + gy**2 + 1e-6)  # gradient magnitude

def gradient_loss_fn(pred, target):
    grad_pred = sobel_xy(pred)
    grad_target = sobel_xy(target)
...
def main():
    ...
    vae = AutoencoderKLTemporalDecoder.from_pretrained(
        args.pretrained_model_name_or_path, subfolder="vae", revision=args.revision, variant="fp16", local_files_only=True, low_cpu_mem_usage=False,)
    ...
    for name, param in vae.named_parameters():
        if 'decoder' in name and 'temporal' not in name and 'time' not in name:
    ...
    if isinstance(vae, (torch.nn.DataParallel, torch.nn.parallel.DistributedDataParallel)):
        vae = vae.module
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(vae):
                ...
                bokeh_pixel_values = batch["bokeh_pixel_values"].to(accelerator.device, non_blocking=True) # [b,f,3,h,w]
                ...
                coc = batch["coc"].to(accelerator.device, non_blocking=True)
                ...
                latents = tensor_to_vae_latent(bokeh_pixel_values, vae).to(weight_dtype)  # [b,f,4,h/8,w/8]
                conditional_latents = tensor_to_vae_latent(coc, vae).to(weight_dtype)  # [b,f,4,h/8,w/8]
                aif_latents = tensor_to_vae_latent(pixel_values, vae, cache_flag=True).to(weight_dtype)  # [b,f,4,h/8,w/8]
                ...
                with accelerator.autocast():
                    model_pred = decode_latents(vae, model_pred, args.num_frames)  # [batch, frames, channels, height, width]

                loss = torch.abs((model_pred.float() - bokeh_pixel_values)).mean()
                if args.texture_loss:
                    gradient_loss = gradient_loss_fn(model_pred.flatten(0,1).float(),  bokeh_pixel_values.flatten(0,1))
                    loss += gradient_loss
            ...
            if accelerator.sync_gradients:
                progress_bar.update(1)
                global_step += 1
                tensorboard_log = {"train_loss": train_loss}
                if args.texture_loss:
                    tensorboard_log.update({"gradient_loss": gradient_loss.item()})

                accelerator.log(tensorboard_log, step=global_step)
                train_loss = 0.0
